...
(defun preprocess (expr)
  (let ((stack '())
    ...
    (format t "Size of stack : ~a~%" (length stack))
    (reverse stack)))
...
(defun add-code (expression)
  ...
  (if expression
      (cond ((equal *target* 'code)
             (setf *code_list* (concatenate 'list *code_list* expression)))
            ((equal *target* 'implementation)
             (setf *implementation_list*
                   (concatenate 'list *implementation_list*
                                                 expression)))
            ((equal *target* 'definition)
             (setf *definition_list* (concatenate 'list *definition_list*
                                             expression))))))
        ...
        (if (equal "(" (car *expr-list*))
            (let ((space nil))
              ...
              (if (and (not space) (not omit))
                  (add-code (format nil ";~%")))
              (setf *expressions* (1+ *expressions*))
              (sb-ext:gc)
...
(defun parse (expression)
  ...
  (let ((expr-list (preprocess expression)))
    (setq *expr-list* expr-list)
    (setf *code_list* '(""))
    (setf *implementation_list* '(""))
    (setf *definition_list* '(""))
    (setf *paranteses* 0)
    (setf  *block* 0)
    (setf *variables* (make-hash-table :test 'equal :size 10000))
    (setf *functions* (make-hash-table :test 'equal :size 500))
    (setf *function-map* (make-hash-table :test 'equal :size 500))
    (setf *signatures* (make-hash-table :test 'equal :size 500))
    (setf *current-function* (make-hash-table :test 'equal :size 500))
    (setup-signatures)
    (loop while (and (> (length expr-list) 0)
                     *expr-list*) do
         (if *expr-list*
             (progn
               (parse-expression))))
