...
(defun parse-if-open (expr-list)
  ...
  (add-code (format nil "{~%"))
  (return-from parse-if-open expr-list))

(defun parse-if-close (expr-list)
  (add-code (format nil "}~%"))
  (if (not (equal ")" (car expr-list)))
      (progn
        (add-code "else")
  ...
  (return-from parse-if-close expr-list))
...
(Defun parse-for-open (expr-list)
    (zero-arg)
  (inc-block)
  ...
  (setf expr-list (parse-close-square-bracket expr-list))
  (add-code (format nil "{~%"))
  (return-from parse-for-open expr-list))

(defun parse-for-close (expr-list)
  (dec-block)
  (add-code (format nil "}~%"))
  (dbg "parse-for BLOCK END  block " *block* " parens " *paranteses*)
  (dbg "parse-for " expr-list)
  (return-from parse-for-close expr-list))
...
(defun parse-let-open (expr-list)
  (zero-arg)
  (inc-block)
  ...
  (setf expr-list (parse-let-vector expr-list))
  (setf expr-list (parse-close-square-bracket expr-list))
  (return-from parse-let-open expr-list))

(defun parse-let-close (expr-list)
  (dec-block)
  (add-code (format nil "}~%"))
  (dbg "parse-let BLOCK END  block " *block* " parens " *paranteses*)
  (dbg "parse-let " expr-list)
  (return-from parse-let-close expr-list))
...
(defun parse-defn-function-open (expr-list)
  (setf *fn-name* (car expr-list))
  (zero-arg)
  ...
  (add-code ")")
  (add-code (format nil "~%{~%"))
  (return-from parse-defn-function-open expr-list))

(defun parse-defn-function-close (expr-list)
  ...
  (return-from parse-defn-function-close expr-list))
...
(defun parse-call (expr-list)
  (cond ((equal "(" (car expr-list))
        ...
        ((equal "let" (car expr-list))
         ...
         (setf expr-list (parse-let-open (cdr expr-list)))
         (setf expr-list (parse-block expr-list))
         (setf expr-list (parse-let-close expr-list))
        ...
        ((or (equal "for" (car expr-list)) (equal "∑" (car expr-list)))
         ...
         (setf expr-list (parse-for-open (cdr expr-list)))
         (setf expr-list (parse-block expr-list))
         (setf expr-list (parse-for-close expr-list))
         (return-from parse-call expr-list))
        ((equal "defn" (car expr-list))
         ...
         (setf expr-list (parse-defn-function-open (cdr expr-list)))
         (setf expr-list (parse-block expr-list))
         (setf expr-list (parse-defn-function-close expr-list))
         (return-from parse-call expr-list))
        ...
        ((equal "if" (car expr-list))
         ...
         (setf expr-list (parse-if-open (cdr expr-list)))
         (setf expr-list (parse-expression expr-list))
         (setf expr-list (parse-if-close expr-list))
         (return-from parse-call expr-list))               
