  ...
  (let ((operator "")
    ...
    (cond ((equal "\n" (car expr-list))
          ...
          ((not (equal "\n" (car expr-list)))
           ...
           (if (and (not (remove-if-not #'(lambda (x)
                            (equal x (get-iter-variable-name (car left))))
                                   (hash-table-keys *variables*)))
                    (not (numberp (parse-integer (car left) :junk-allowed t))))
               (remove-last-code (format nil ";~%")))
           (dbg "GET LAST CODE " (get-last-code))
           ...
           (setf expr-list (parse-expression expr-list))
            (if (and (not (remove-if-not #'(lambda (x)
                            (equal x (get-iter-variable-name (car right))))
                                         (hash-table-keys *variables*)))
                     (not (numberp (parse-integer (car right) :junk-allowed t))))
                (remove-last-code (format nil ";~%")))
           (dbg "GET LAST CODE " (get-last-code))
          ...
          ((find #\: (car expr-list))
           (let ((def (split ":" (car expr-list))))
             ...
             (if (or (remove-if-not #'(lambda (x)
                            (equal x (get-iter-variable-name (cadr from))))
                                    (hash-table-keys *variables*))
                     (numberp (parse-integer (cadr from) :junk-allowed t)))
                 (add-code (format nil ";~%")))
             (add-code "for")
             (add-code "(")
             (add-code var-name)
             (add-code "=")
             (setf expr-list (parse-expression (cdr from)))
             (if (or (remove-if-not #'(lambda (x)
                            (equal x (get-iter-variable-name (cadr from))))
                                    (hash-table-keys *variables*))
                     (numberp (parse-integer (cadr from) :junk-allowed t)))
             ...
             (setf expr-list (parse-expression expr-list))
             (if (or (remove-if-not #'(lambda (x)
                            (equal x (get-iter-variable-name (car to))))
                                    (hash-table-keys *variables*))
                     (numberp (parse-integer (car to) :junk-allowed t)))
                 (add-code (format nil ";")))
             (add-code var-name)
