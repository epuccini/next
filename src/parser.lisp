...
(defun parse-function-type (expr-list &optional (signature nil))
  (let ((fn-name (car (split ":" (car expr-list))))
    ...
    (cond ((equal "i16" type)
          ...
          ((equal "cstring" type)
           (add-code "const char*")
           (setf (gethash (get-function-name fn-name) *functions*) 'const-char-array)
           (setf expr-list (cdr expr-list)))
          ...
          ((equal "string>" type)
           (add-code "char**")
           (setf (gethash (get-function-name fn-name) *functions*) 'char-array-pointer)
           (setf expr-list (cdr expr-list)))
          ((equal "cstring>" type)
           (add-code "const char**")
           (setf (gethash (get-function-name fn-name) *functions*) 'const-char-array-pointer)
           (setf expr-list (cdr expr-list)))
...
(defun parse-variable-type (var-name type-name &optional (signature nil))
  (cond ((equal "i16" type-name)
        ...
        ((equal "string>" type-name)
         (add-code "string*")         
         (setf (gethash (get-variable-name var-name) *variables*) 'char-array-pointer))
        ((equal "cstring>" type-name)
         (add-code "cstring*")         
         (setf (gethash (get-variable-name var-name) *variables*) 'const-char-array-pointer))
...
(defun parse-call (expr-list)
  (cond ((equal ")" (car expr-list))
        ...
        ((equal "open" (car expr-list))
         (parse-math expr-list "open" 2))
        ((equal "read-line" (car expr-list))
         (parse-math expr-list "read_line" 1))
        ((equal "write-line" (car expr-list))
         (parse-math expr-list "write_line" 2))
        ((equal "read-binary" (car expr-list))
         (parse-math expr-list "read_binary" 4))
        ((equal "write-binary" (car expr-list))
         (parse-math expr-list "write_binary" 4))
        ((equal "close" (car expr-list))
         (parse-math expr-list "close" 1))
