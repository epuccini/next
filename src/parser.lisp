...
(defun error-missing-square-bracket ()
  ...
  (cond ((find #\: (car expr-list))
         (let ((def (split ":" (car expr-list))))
           ...
           (cond ((equal "int16" (cadr def))
                  (add-code "short")
                  (setf (gethash (get-iter-variable-name (car def)) *variables*) 'integer))
                 ((equal "int32" (cadr def))
                  (add-code "int")
                  (setf (gethash (get-iter-variable-name (car def)) *variables*) 'integer))
                 ((equal "int64" (cadr def))
                  (add-code "long")
                  (setf (gethash (get-iter-variable-name (car def)) *variables*) 'bigint))
                 ((equal "float32" (cadr def))
                  (add-code "float")
                  (setf (gethash (get-iter-variable-name (car def)) *variables*) 'single-float))
                 ((equal "float64" (cadr def))
                  (add-code "double")
                  (setf (gethash (get-iter-variable-name (car def)) *variables*) 'double-float))
                 ((equal "bool" (cadr def))
                  (add-code "bool")
                  (setf (gethash (get-iter-variable-name (car def)) *variables*) 'boolean))
                 ((equal "byte" (cadr def))
                  (add-code "char")
                  (setf (gethash (get-iter-variable-name (car def)) *variables*) 'char))
                 ((equal "string" (cadr def))
                  (add-code "char*")
                  (setf (gethash (get-iter-variable-name (car def)) *variables*) 'string)))
           (add-code " ")
