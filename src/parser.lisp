...
(defun parse-inner-block (expr-list)
  (cond ((equal "(" (car expr-list))
         (progn
           (inc-parens)
           (format t "parse-inner-block: OPEN INNER Block: ~a~%" *paranteses*)
        ...
        ((equal ")" (car expr-list))
         (progn
           ...
           (if (= (- *paranteses* *block*) 0)
               (progn
                 (format t "parse-inner-block: RET INNER ) Block: ~a~%" *paranteses*)
                 (return-from parse-inner-block expr-list))
               (progn
                 (format t "parse-inner-block: CLOSE INNER ) Block: ~a~%" *paranteses*)
        ...
        ((> (length expr-list) 0)
         (format t "parse-inner-block: PARSE INNER ~a Block: ~a~%" (cdr expr-list) *paranteses*)
         (setf expr-list (parse-inner-block (cdr expr-list)))))
  ...
  (cond ((equal "(" (car expr-list))
         (progn
           ...
           (if (and *paranteses* (not (equal "(" (car (last *call*)))))
               (setf *call* (append *call* (list ","))))
           (format t "parse-arguments: OPEN ARG ( Block: ~a~%" *paranteses*)
           (setf expr-list (parse-call (cdr expr-list)))))
        ((equal "\"" (car expr-list))
         (if (and *paranteses*
        ...
        ((equal ")" (car expr-list))
         (progn
           (dec-arg)
           (dec-parens)
           (format t "parse-arguments: ARG CLOSE ) Block ~a~%" *paranteses*)
           (setf *call* (append *call* (list ")")))
           (if (= (- *paranteses* *block*) 0)
               (setf *call* (append *call* (list (format nil ";~%")))))
           (dec-arg)
...
(defun parse-call (expr-list)
   (cond ((equal "(" (car expr-list))
          (progn
            (inc-parens)
            (format t "parse-call: CALL OPEN ( Block: ~a~%" *paranteses*)
            (setf expr-list (parse-call (cdr expr-list)))))
         ((equal ")" (car expr-list))
         (progn
           (dec-parens)
           (format t "parse-call: CALL CLOSE ) Block: ~a~%" *paranteses*)
           (if (> (- *paranteses* *block*) 0)
               (setf *call* (append *call* (list ")"))))
           (if (= (- *paranteses* *block*) 0)
               (setf *call* (append *call* (list (format nil ");~%")))))
...
(defun parse-expression (expr-list)
  (cond ((equal "(" (car expr-list))
         (progn
           (inc-arg)
           (inc-parens)
           (format t "parse-expression: OPEN EXPR ( Block: ~a~%" *paranteses*)
           (setf expr-list (parse-call (cdr expr-list)))))
        ((not (car expr-list))
         (format t "parse-expression: NIL~%"))
        ((equal ")" (car expr-list))
         (dec-parens)
         (format t "parse-expression: CLOSE EXPR ( Block: ~a~%" *paranteses*)
         (if (= (- *paranteses* *block*) 0)
             (setf *call* (append *call* (list (format t ";~%")))))
         (format t "parse-expression: CALL CLOSE Block: ~a~%" *paranteses*))
        ((equal (format nil "~a" #\Tab) (car expr-list))
         (setf expr-list (cdr expr-list)))
        ((numberp (parse-integer (car expr-list) :junk-allowed t))
         (format t "parse-expression: NUM Block: ~a~%" *paranteses*)
         (setf *call* (append *call* (list (car expr-list))))
         (setf *call* (append *call* (list (format nil ";~%"))))
         (setf expr-list (cdr expr-list)))
        ((equal "\"" (car expr-list))
         (format t "parse-expression: STRING Block: ~a~%" *paranteses*)
         (setf *call* (append *call* (list "\"")))
         (setf expr-list (parse-cstr (cdr expr-list)))
