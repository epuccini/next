   ...
   (cond ((equal "(" (car expr-list))
        ...
        ((equal "println" (car expr-list))
         (let ((tp1 (gethash (get-variable-name (cadr expr-list)) *variables*))
               ...
               (tp3 (inspect-type (cdr expr-list)))
               (tp nil))
           (cond ((remove-if-not #'(lambda (x)
                 ...
                 (tp2
                  (setf tp tp2)))
           (format t "parse-arguments: println TP2 ~a~%" tp2)
           (format t "parse-arguments: println TP3 ~a~%" tp3)
        ...
        ((equal "return" (car expr-list))
         (setf *call* (append *call* (list "return" "(")))
         (zero-arg)
         (setf expr-list (parse-arguments (cdr expr-list) 1)))
        ((remove-if-not #'(lambda (x)
