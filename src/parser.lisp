...
(defun parse-argument-vector (expr-list)
  (cond ((equal "[" (car expr-list))
        ...
        ((equal "]" (car expr-list))
         ...
         (add-code ")")
         (add-code (format nil "~%{~%"))
         (dbg "parse-argument-vector: OPEN BRACKET")
...
(defun parse-inner-block (expr-list)
  ...
  (if (not (equal ")" (car expr-list)))
      (progn
        (dbg "parse-inner-block. EXIT" expr-list)
        (setf expr-list (cdr expr-list))))
  expr-list)
...
(defun parse-call (expr-list)
  (cond ((equal "(" (car expr-list))
        ...
        ((equal ")" (car expr-list))
         (progn
           ...
           (if (> (- *paranteses* *block*) 0)
               (progn
                 (dbg "parse-call: CALL INNER CLOSE ) Block " *block*)
                 (add-code ")")))
           (if (= *paranteses* *block*)
               (progn
                 (dbg "parse-call: CALL OUTER CLOSE ) Block " *block*)
                 (add-code (format nil ");~%"))))
           (dec-arg)
        ...
        ((equal "defn" (car expr-list))
         ...
         (dbg "parse-call: PARENS " *paranteses*)
         (zero-parens)
