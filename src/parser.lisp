  ...
  (let ((operator "")
    ...
    (cond ((equal "\n" (car expr-list))
          ...
          ((not (equal "\n" (car expr-list)))
           ...
           (if (not (numberp (parse-integer (car left) :junk-allowed t)))
           ...
           (setf expr-list (parse-expression expr-list))
            (if (not (numberp (parse-integer (car right) :junk-allowed t)))
                (remove-last-code (format nil ";~%")))
           (dbg "GET LAST CODE " (get-last-code))
    ...
    (cond ((equal "\n" (car expr-list))
          ...
          ((find #\: (car expr-list))
           (let ((def (split ":" (car expr-list))))
             ...
             (setf from expr-list)
             (setf expr-list (parse-expression (cdr expr-list)))
             (if (numberp (parse-integer (cadr from) :junk-allowed t))
                 (add-code (format nil ";~%")))
             (add-code "for")
             (add-code "(")
             (add-code var-name)
             ...
             (setf to expr-list)
             (setf expr-list (parse-expression expr-list))
             (if (numberp (parse-integer (car to) :junk-allowed t))
                 (add-code (format nil ";")))
             (add-code var-name)
