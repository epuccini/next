...
(defun parse-inner-block (expr-list)
  (cond ((equal "(" (car expr-list))
         (progn
           (print "parse-inner-block: OPEN INNER")
           (inc-arg)
           (inc-block)
           (parse-call (cdr expr-list))))
        ((equal ")" (car expr-list))
         (progn
           (print "parse-inner-block: CLOSE INNER")
           (dec-arg)
           (dec-block)
           (parse-call (cdr expr-list))))
        ((equal "\n" (car expr-list))
        ...
        ((> (length (cdr expr-list)) 0)
         (format t "parse-inner-block: EXPRE ;~a;" (cdr expr-list))
...
(defun parse-parameter-vector (expr-list)
  (cond ((equal "[" (car expr-list))
        ...
        ((equal "]" (car expr-list))
         (print "parse-parameter-vector: CLOSE VEC")
...
(defun parse-arguments (expr-list max)
  (cond ((equal "(" (car expr-list))
        ...
        ((equal ")" (car expr-list))
         (progn
           (print "parse-arguments: ARG CLOSE")
           (dec-block)
           (setf *call* (append *call* (list ")")))
           (if (= *block* 0)
               (setf *call* (append *call* (list (format nil ";~%")))))
