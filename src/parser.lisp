  ...
  (cond ((equal "\n" (car expr-list))
        ...
        ((find #\: (car expr-list))
         (let ((def (split ":" (car expr-list))))
           ...
           (add-code " ")
           (add-code (get-variable-name (car def)))
           (format t "parse-variable: parsed ~a~%"
           ...
           (cond ((> (- *paranteses* *block*) 0)
                  (progn
                    (if (and
                         ...
                         (not (equal "int32" fn))
                         (not (equal "int64" fn))
                         (not (equal "float32" fn))
                         (not (equal "float64" fn))
                         (not (equal "bool" fn))
                         (not (equal "byte" fn))
                         (not (equal "char" fn)))
                        (add-code ")"))
        ...
        ((numberp (parse-integer (car expr-list) :junk-allowed t))
         (progn
           ...
           (if (and (get-arg)
               ...
               (add-code ","))
           (if (> (get-arg) max)
               (error-parameter-max (get-arg) max))
           (progn
...
(defun parse-call (expr-list)
  (cond ((equal "(" (car expr-list))
        ...
        ((equal "bool" (car expr-list))
         ...
         (setf expr-list (parse-arguments (cdr expr-list) 1))
         (return-from parse-call expr-list))
        ((equal "byte" (car expr-list))
         (store-current-function "byte")
         (add-code "(")
         (add-code "byte")
         (add-code ")")
         (zero-arg)
         (setf expr-list (parse-arguments (cdr expr-list) 1))
         (return-from parse-call expr-list))
        ((equal "int16" (car expr-list))
         (store-current-function "int16")
         (add-code "(")
         (add-code "int16")
         (add-code ")")
         (zero-arg)
         (setf expr-list (parse-arguments (cdr expr-list) 1))
         (return-from parse-call expr-list))
        ((equal "int32" (car expr-list))
         (store-current-function "int32")
         (add-code "(")
         (add-code "int32")
         (add-code ")")
         (zero-arg)
         (setf expr-list (parse-arguments (cdr expr-list) 1))
         (return-from parse-call expr-list))
        ((equal "int64" (car expr-list))
         (store-current-function "int64")
         (add-code "(")
         (add-code "int64")
         (add-code ")")
         (zero-arg)
         (setf expr-list (parse-arguments (cdr expr-list) 1))
         (return-from parse-call expr-list))
        ((equal "float64" (car expr-list))
         (store-current-function "float64")
         (add-code "(")
         (add-code "float64")
         (add-code ")")
         (zero-arg)
         (setf expr-list (parse-arguments (cdr expr-list) 1))
         (return-from parse-call expr-list))
        ((equal "float32" (car expr-list))
         (store-current-function "float32")
         (add-code "(")
         (add-code "float32")
         (add-code ")")
         (zero-arg)
