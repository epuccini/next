...
(defun parse-inner-let-block (expr-list)
  (if (equal "(" (car expr-list))
      (progn
        (setf expr-list (parse-expression expr-list))
        (setf expr-list (parse-inner-let-block expr-list))))
  (if (not expr-list)
      (return-from parse-inner-let-block expr-list))
  (if (equal "\n" (car expr-list))
      (setf expr-list (parse-inner-let-block (cdr expr-list))))
  (if (equal ")" (car expr-list))
      (progn
        (dec-parens)
        (dbg "parse-inner-let-block: CLOSE" expr-list)
        (return-from parse-inner-let-block expr-list)))
  (if (not (equal ")" (car expr-list)))
      (progn
        (dbg "parse-inner-let-block " (car expr-list))
        (setf expr-list (parse-inner-let-block (cdr expr-list))))))
