...
(defun parse-inner-block (expr-list)
  (cond ((equal "(" (car expr-list))
        ...
        ((> (length (cdr expr-list)) 0)
         (format t "parse-inner-block: EXPR ~a Block: ~a~%" (cdr expr-list) *block*)
         (parse-expression (cdr expr-list))
         (format t "parse-innner-block: SKIP~%")
         (setf expr-list (skip-expr-or-value (cdr expr-list) 0))
         (format t "parse-inner-block: OPEN INNER ~a Block: ~a~%" (cdr expr-list) *block*)
         (if (car expr-list)
             (parse-inner-block expr-list)))))
...
(defun parse-variable (expr-list)
  (cond ((find #\: (car expr-list))
         (let ((def (split ":" (car expr-list))))
           ...
           (setf *call* (append *call* (list "=")))
           (format t "parse-variable: OPEN ARG ~a~%" (cadr expr-list))
           (parse-expression (cdr expr-list))))))
...
(defun parse-parameter-vector (expr-list)
  (cond ((equal "[" (car expr-list))
        ...
        ((equal "]" (car expr-list))
         ...
         (parse-inner-block (cdr expr-list))
         (return-from parse-parameter-vector))
        ((equal "(" (car expr-list))
         (inc-block)
        ...
        ((stringp (car expr-list))
         (parse-variable (cdr expr-list))
         (setf expr-list (skip-expr-or-value (cdr expr-list) 0))
         (format t "parse-parameter-vector: NEXT VAR ~a~%" (cdr expr-list))
         (parse-parameter-vector (cdr expr-list)))))
...
(defun parse-call (expr-list)
   (cond ((equal "(" (car expr-list))
         ...
         ((equal ")" (car expr-list))
         (progn
           (format t "parse-call: CALL CLOSE~%")
           (dec-block)
           (if (= *block* 0)
               (setf *call* (append *call* (list (format nil ";~%")))))
           (if (> *block* 0)
...
(defun parse-expression (expr-list)
  (cond ((equal "(" (car expr-list))
         (progn
           (inc-arg)
           (inc-block)
           (print "parse-expression: OPEN EXPR")
           (parse-call (cdr expr-list))
           (print "parse-expression: CLOSE EXPR")))
        ((not (car expr-list))
        ...
        ((numberp (parse-integer (car expr-list) :junk-allowed t))
         ...
         (setf *call* (append *call* (list (format nil ";~%")))))
        ((equal "\"" (car expr-list))
            (format t "parse-call: STGRING Block: ~a~%" *block*)
         (if (and *block*
                  (not (equal "(" (car (last *call*)))))
             (setf *call* (append *call* (list ","))))
         (setf *call* (append *call* (list "\"")))
