...
(defun parse-inner-block (expr-list)
  (cond ((equal "(" (car expr-list))
         (progn
           (inc-parens)
           (dbg "parse-inner-block: OPEN INNER Block " *paranteses*)
        ...
        ((> (length expr-list) 0)
         (dbg "parse-inner-block: PARSE INNER " (cdr expr-list) " Block " *paranteses*)
         (setf expr-list (parse-inner-block (cdr expr-list)))))
...
(defun parse-parameter-vector (expr-list)
  (cond ((equal "[" (car expr-list))
        ...
        ((equal "(" (car expr-list))
         (inc-parens)
         (dbg "parse-parameter-vector: ( Block " *paranteses*)
  ...
  (cond ((equal "(" (car expr-list))
         (progn
           ...
           (if (and *paranteses* (not (equal "(" (car (last *call*)))))
               (setf *call* (append *call* (list ","))))
           (dbg "parse-arguments: OPEN ARG ( Block " *paranteses*)
           (setf expr-list (parse-call (cdr expr-list)))))
        ((equal "\"" (car expr-list))
         (if (and *paranteses*
        ...
        ((equal ")" (car expr-list))
         (progn
           (dec-arg)
           (dec-parens)
           (dbg "parse-arguments: ARG CLOSE ) Block " *paranteses*)
           (setf *call* (append *call* (list ")")))
           (if (= (- *paranteses* *block*) 0)
               (setf *call* (append *call* (list (format nil ";~%")))))
           (dec-arg)
        ...
        ((remove-if-not #'(lambda (x) (equal x (car expr-list)))
         ...
         (progn
           ...
           (if (> (gethash *paranteses* *arguments*) max)
               (error-parameter-max (gethash *paranteses* *arguments*)
                                    max))
           (dbg "parse-arguments: VARIABLE " (car expr-list))
           (setf *call* (append *call* (list (car expr-list))))
           (setf expr-list (parse-arguments (cdr expr-list) max)))))
  expr-list)
...
(defun parse-call (expr-list)
   (cond ((equal "(" (car expr-list))
          (progn
            (inc-parens)
            (dbg "parse-call: CALL OPEN ( Block " *paranteses*)
            (setf expr-list (parse-call (cdr expr-list)))))
         ((equal ")" (car expr-list))
         (progn
           (dec-parens)
           (dbg "parse-call: CALL CLOSE ) Block " *paranteses*)
           (if (> (- *paranteses* *block*) 0)
               (setf *call* (append *call* (list ")"))))
           (if (= (- *paranteses* *block*) 0)
               (setf *call* (append *call* (list (format nil ");~%")))))
...
(defun parse-expression (expr-list)
  (cond ((equal "(" (car expr-list))
         (progn
           (inc-arg)
           (inc-parens)
           (dbg "parse-expression: OPEN EXPR ( Block " *paranteses*)
           (setf expr-list (parse-call (cdr expr-list)))))
        ((not (car expr-list))
         (dbg "parse-expression: NIL"))
        ((equal ")" (car expr-list))
         (dec-parens)
         (dbg "parse-expression: CLOSE EXPR ( Block " *paranteses*)
         (if (= (- *paranteses* *block*) 0)
             (setf *call* (append *call* (list (format t ";~%")))))
         (dbg "parse-expression: CALL CLOSE Block " *paranteses*))
        ((equal (format nil "~a" #\Tab) (car expr-list))
         (setf expr-list (cdr expr-list)))
        ((numberp (parse-integer (car expr-list) :junk-allowed t))
         (dbg "parse-expression: NUM Block " *paranteses*)
         (setf *call* (append *call* (list (car expr-list))))
         (setf *call* (append *call* (list (format nil ";~%"))))
         (setf expr-list (cdr expr-list)))
        ((equal "\"" (car expr-list))
         (dbg "parse-expression: STRING Block " *paranteses*)
         (setf *call* (append *call* (list "\"")))
         (setf expr-list (parse-cstr (cdr expr-list)))
