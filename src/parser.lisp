...
(defun is-fixed-math-type-p (type)
  (or (search "f32" type) (search "f64" type) (search "f80" type)
      (search "i16" type) (search "i32" type) (search "i64" type)
...
(defun set-insert-main-points ()
  (setf *main-start-code* (length *code_list*))
...
(defun insert-main-buffer ()
  (if (equal *target* 'code)
      (progn
        (dbg "Insert main buffer into code")
        (setf *code_list* (list-into-list
                           *code_list*
                           *main_buffer*
                           *main-start-code*))
        (setf *main-start-code*
              (+ *main-start-code*
                 (length *main_buffer*)))))
  (if (equal *target* 'implementation)
      (progn
        (dbg "Insert main buffer into impl")
...
(defun add-bigint-declaration-with-var (tmp-var var-name type)
  (setf (gethash (get-variable-name tmp-var) *variable-type*) type)
  (add-code "mpz_t")
  (add-code " ")
  (add-code tmp-var)
  (add-code (format nil ";~%"))
...
(defun add-variable-value (expr-list var-name)
  (let ((tmp-target *target*)
        ...
        ((or (is-variable-split-name-p (car expr-list))
         ...
         (let ((comp-name (build-dotted-type (car expr-list)))
           ...
           (if (search ">>" (car expr-list))
               (add-code comp-name)
               (add-variable-value expr-list var-name))
           (setf expr-list (parse-arguments (cdr expr-list) max))
           (return-from parse-arguments expr-list)))
...
(defun parse-call-function (expr-list)
  (let* ((fn-name (car expr-list))
         (ret-type (get-iter-function-type fn-name))
         (type (get-next-token-type-string (cdr expr-list)))
         (fn-type (get-iter-function-args fn-name))
         (tmp-target *target*))
...
(defun parse-call (expr-list)
  ...
  (cond ((equal ")" (car expr-list))
         ...
         (let ((tmp-target *target*)
           ...
           (if (equal (cadr def) "ixx")
               (progn
                 ...
                 (setf expr-list (cdr expr-list))
                 ;; switch target - clear buffer
                 (set-target 'mainbuffer)
                 ;; move to value)
                 (setf expr-list (cdr expr-list))
                 ;; add initialisation
                 (dbg "parse-call: add-bigint-init")
                 (add-bigint-init (car def) (car expr-list) "i64")
                 (set-target tmp-target)
                 ;; move to end
                 (setf expr-list (cdr expr-list))
        ...
        ((or (equal "shitfr" (car expr-list))
         ...
         (return-from parse-call expr-list))
        ((is-iter-function-p (car expr-list))
         (setf expr-list (parse-call-function expr-list)))
        ((is-iter-variable-p (car expr-list))
