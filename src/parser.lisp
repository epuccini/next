  ...
  (cond ((equal "(" (car expr-list))
        ...
        ((equal ")" (car expr-list))
         (let ((fn (get-current-function)))
           (dbg "parse-arguments: ) " (car expr-list) " max " max)
           (dec-arg)
           ...
           (cond ((> (- *paranteses* *block*) 0)
                  (progn
                    (if (and
                         ...
                         (not (equal "char" fn)))
                        (add-code ")"))
                    ;(if (equal (get-arg) max)
                        ;(progn
                          ;(dbg "parse-arguments: call end max " max)
                          ;(setf expr-list (cdr expr-list))
                          ;(return-from parse-arguments expr-list)))
                    (dbg "parse-arguments: more params max " max)
                    (setf expr-list (cdr expr-list))
  ...
  (cond ((equal "(" (car expr-list))
        ...
        ((or (equal "elt" (car expr-list)) (equal "#" (car expr-list)))
         ...
         (progn
           ...
           (add-code "(")
           (zero-arg)
           (dbg "parse-call: prnfmt Next arg " (cdr expr-list))
           (setf expr-list (parse-arguments (cdr expr-list) *infinite-arguments*))))
        ((equal "prnl" (car expr-list))
         (let ((type (get-type expr-list)))
           ...
           (add-code "(")
           (zero-arg)
           (dbg "parse-call: prnl> Next arg " (cdr expr-list))
        ...
        ((equal "prn" (car expr-list))
         (let ((type (get-type expr-list)))
           ...
           (add-code "(")
           (zero-arg)
           (dbg "parse-call: prn Next arg " (cdr expr-list))
           (setf expr-list (parse-arguments (cdr expr-list) 1))))
        ...
        ((or (equal "add" (car expr-list)) (equal "+" (car expr-list)))
         (let ((type (get-math-type expr-list)))
           ...
           (dbg "parse-call: parse-arguments: "
                (format nil "~a" (1- (count-elements expr-list))))
           (setf expr-list (parse-arguments (cdr expr-list)
        ...
        ((or (equal "sub" (car expr-list)) (equal "-" (car expr-list)))
         (let ((type (get-math-type expr-list)))
           ...
           (zero-arg)
           (add-code (format nil "~a" (1- (count-elements expr-list))))
           (setf expr-list (parse-arguments (cdr expr-list) (count-elements expr-list)))
           (return-from parse-call expr-list)))
