...
(defun parse-argument-vector (expr-list)
  (cond ((equal "[" (car expr-list))
        ...
        ((equal "]" (car expr-list))
         ...
         (add-code ")")
         (add-code (format nil "~%{~%"))
         (dbg "parse-argument-vector: OPEN BRACKET block " *block* " Parens " *paranteses*)
         (if (cdr expr-list)
             (setf expr-list (parse-inner-block (cdr expr-list))))
         (dbg "parse-argument-vector: EXIT with " expr-list)
         (return-from parse-argument-vector expr-list))
        ((equal "(" (car expr-list))
         (inc-parens)
         (dbg "parse-argument-vector: ( block " *block* " Parens " *paranteses*)
...
(defun parse-function (expr-list)
  (cond ((stringp (car expr-list))
         (let ((fn-name (car (split ":" (car expr-list)))))
           ...
           (setf expr-list (parse-argument-vector expr-list)))))
                                        ;(setf expr-list (parse-rest-function expr-list)
  (dbg "parse-function: EXIT Rest. " expr-list)
  (add-code (format nil "}~%"))
  (dec-block)
  (setf *target* 'code)
  expr-list)
...
(defun parse-inner-block (expr-list)
  (cond ((equal "(" (car expr-list))
         (progn
           (inc-parens)
           (dbg "parse-inner-block: OPEN INNER block " *block* " Parens " *paranteses*)
           (setf expr-list (parse-call (cdr expr-list)))))
        ((equal ")" (car expr-list))
         (progn
           (dec-parens)
           (dbg "parse-inner-block: ) block "
                      *block* " Parens " *paranteses*)
           (if (= *paranteses* *block*)
               (progn
                 (dbg "parse-inner-block: RET INNER ) block "
                      *block* " Parens " *paranteses*)
                 (return-from parse-inner-block expr-list))
               (progn
                 (dbg "parse-inner-block: CLOSE INNER ) block "
                      *block* " Parens " *paranteses*)
                 (setf expr-list (parse-inner-block (cdr expr-list)))))))
        ((equal "\n" (car expr-list))
         (dbg "parse-inner-block: RET INNER " (cdr expr-list))
         (setf expr-list (parse-inner-block (cdr expr-list))))
        ((equal ";|" (car expr-list))
        ...
        ((> (length expr-list) 0)
         (dbg "parse-inner-block: PARSE INNER "
              (cdr expr-list) " block " *block* " Parens " *paranteses*)
         (setf expr-list (parse-inner-block (cdr expr-list)))))
  (if (equal "(" (car expr-list))
      (progn
...
(defun parse-parameter-vector (expr-list)
  (cond ((equal "[" (car expr-list))
        ...
        ((equal "]" (car expr-list))
         ...
         (setf expr-list (parse-inner-block (cdr expr-list)))
         (dbg "parse-parameter-vector: CLOSE BRACKET")
         (add-code (format nil "}~%"))
         (dec-block)
...
(defun parse-call (expr-list)
  (cond ((equal "(" (car expr-list))
        ...
        ((equal "println" (car expr-list))
         (let ((variable-type (gethash (get-variable-name (cadr expr-list)) *variables*))
        ...
        ((equal "defn" (car expr-list))
         ...
         (zero-arg)
         (zero-parens)
         (dbg "parse-call: EXIT parse-function " (cdr expr-list)))
...
(defun parse-expression (expr-list)
  (cond ((equal "(" (car expr-list))
         (progn
           (inc-arg)
           (inc-parens)
           (dbg "parse-expression: OPEN EXPR ( block " *block* " parens " *paranteses*)
           (setf expr-list (parse-call (cdr expr-list)))))
        ((not (car expr-list))
