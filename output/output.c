...
static ixx den_0;
static ixx num_0;
void extract_digit_0_ixx(ixx result_1,ui64 nth_1)
{
mpz_t G547;
mpz_init(G547);
mpz_mul_si(G547,num_0,nth_1);
...
{
mpz_t G554;
mpz_init_set_si(G554,10);
mpz_t G557;
mpz_init(G557);
mpz_t G560;
mpz_init(G560);
mpz_mul_si(G560,den_0,d_1);
mpz_sub(G557,acc_0,G560);
mpz_mul(G554,G554,G557);
...
ui64 k2_2=(1+(k_1*2));
mpz_t G566;
mpz_init_set_ui(G566,k2_2);
mpz_t G569;
mpz_init(G569);
mpz_t G572;
mpz_init(G572);
mpz_t G573;
mpz_init_set_si(G573,2);
mpz_mul(G572,num_0,G573);
mpz_add(G569,acc_0,G572);
mpz_mul(G566,G566,G569);
...
extract_digit_0_ixx(result_2,3);
set_pointer_ui64(&d_2,mpz_get_ui(result_2));
extract_digit_0_ixx(result_2,4);
